...
class Obj{ //宣告一個類別,針對一個畫的圖案
  ...
  update(){ //移動程式碼內容
    ...
    //this.p.y = this.p.y + this.v.y//y軸目前位置(this.p.y)加上x軸的移動速度(this.v.x)
    this.p.add(this.v) //設定好像量後,使用add就可以與上面兩行指令一樣的效果
    //向量sub==>減號
     

    //知道滑鼠的位置,並建立一個滑鼠的向量
    // let mouseV = createVector(mouseX,mouseY) //把滑鼠的位置轉漢城一個向量值
    // let delta = mouseV.sub(this.p).limit(this.v.mag()*2) //sub計算出滑鼠所載位置向量(mouseV)到物件向量
